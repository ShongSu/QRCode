...
/////////////////////////////////////////////////////////////////////////////
// QR版本号1-40：{n,//Ver.n       //P133
//				  数据容量,数据码字符(L级),(M级),(Q级),(H级)
//				  ?,?,?,?,?,?,?,?
//		          纠错的块数,码字总数,数据码字数(L级)		
//                                              (M级)
//                                              (Q级)
...
//
//
static QR_VERSIONINFO QR_VersonInfo[] = {{0}, // (版本号:Ver.0)
										 { 1, // Ver.1
										    26,   19,   16,   13,    9,
										   0,   0,   0,   0,   0,   0,   0,
										   1,  26,  19,
...
// 字符计数指示符的位数({S, M, L},S:1~9;M:10~26;L:27~40)   P126
static int nIndicatorLenNumeral[]  = {10, 12, 14};//数字模式
static int nIndicatorLenAlphabet[] = { 9, 11, 13};//字母数字模式
static int nIndicatorLen8Bit[]	   = { 8, 16, 16};//8位字节模式
static int nIndicatorLenKanji[]	   = { 8, 10, 12};//汉字模式
...
// nLevel：纠错等级  nVersion：版本号  bAutoExtent  nMaskingNo：掩膜模式  lpsSource： ncSource：
...
BOOL CQR_Encode::EncodeData(int nLevel, int nVersion, BOOL bAutoExtent, int nMaskingNo, LPCSTR lpsSource, int ncSource)
{
	...
	else
	{
		...
		{
			if (bAutoExtent)
				m_nVersion = nEncodeVersion; // 僶乕僕儑儞(宆斣)帺摦奼挘
			else
				return FALSE; // 梕検僆乕僶乕
		}
	}
	...
	// 若已有码字个数小于ncDataCodeWord，则交替用"11101100","00010001"进行填充
...
// 不同模式和版本的字符计数指示符和位数   P126
// 根据版本分为三组，QR_VRESION_L：27~40，QR_VRESION_M：10~26，QR_VRESION_S：1~9
// 
int CQR_Encode::GetEncodeVersion(int nVersion, LPCSTR lpsSource, int ncLength)
{
	...
	for (i = nVerGroup; i <= QR_VRESION_L; ++i)   //QR_VRESION_L=2
	{
...
{
	ZeroMemory(m_nBlockLength, sizeof(m_nBlockLength));  //将指定的内存块清零。

	int i, j;

	// 判断编码模式：数字？字母数字？8位字节？汉字？
	...
	int ncSrcBits, ncDstBits; // 源数据流编码成二进制数据流
	...
	m_ncDataCodeWordBit = 0; // 價僢僩扨埵張棟僇僂儞僞
...
// 获取并返回不同模式和符号版本的字符指示符位数和总字符位数  P127
// 模式指示符4位+其他位数
// 
...
int CQR_Encode::GetBitLength(BYTE nMode, int ncData, int nVerGroup)
{
	...
	switch (nMode)
	{
	case QR_MODE_NUMERAL:     // 如果输入的数据的位数不是3的倍数，所余的1位或2位数字应分别转换为4位（最大可表示9）
		                      // 或7位(最大可表示99)二进制数
	...
	case QR_MODE_8BIT:
		ncBits = 4 + nIndicatorLen8Bit[nVerGroup] + (8 * ncData);   //模式指示符4位+字符计数符+8位字节编码位数（每个字节8位）
		break;

	default: // case QR_MODE_KANJI:
		ncBits = 4 + nIndicatorLenKanji[nVerGroup] + (13 * (ncData / 2));  //模式指示符4位+字符计数符+汉字字节位数（每个汉字13位）
		break;
	}
...
// 对编码内容形式进行判定是否是汉字模式
// 是的话返回TRUE
// Shift-JIS码（即S-JIS码）避开原本在0xA1-0xDF放置的半角假名字符
// 它也是由两个字节组成。高字节是从0x81-0x84，0x87-0x9F，0xE0-0xEA，0xED-0xEE，0xFA-0xFC，
// 低字节是从0x40-0xFC
// 
...
//
// 汉字模式的编码 译码转换  标准P22
// 1． 对于转换JIS值为8140HEX 到 9FFCHEX之间的字符：
// a)	将转换JIS值减去8140HEX；
// b)	将高位字节乘以C0HEX；
// c)	将b的结果加上低位字节；
//
// 2． 转换JIS值为E040HEX 到EBBFHEX之间的字符：
// a）	将转换JIS值减去C140HEX；
// b）	将高位字节乘以C0HEX；
// c）	将b的结果加上低位字节；
//
...
// 计算要编码的数据码字相应的纠错码字
// GF(2^8)有限域  在加罗瓦域上的乘法运算： a^m * a^n = a^(m+n)mod255    
// 除法运算： a^m /a^n = a^(m-n)mod255
// 加减法运算: m+n = mXORn             
...
void CQR_Encode::GetRSCodeWord(LPBYTE lpbyRSWork, int ncDataCodeWord, int ncRSCodeWord)
{
	...
	for (i = 0; i < ncDataCodeWord ; ++i)
	{
		if (lpbyRSWork[0] != 0)
		{
			BYTE nExpFirst = byIntToExp[lpbyRSWork[0]]; // GF(2^8)有限域，生成多项式的各符号的指数数取值

			for (j = 0; j < ncRSCodeWord; ++j)
			{
				// 如果指数a已经超过了255，则 % 255 并令 a^255=1
...
void CQR_Encode::FormatModule()
{
	...
	ZeroMemory(m_byModuleData, sizeof(m_byModuleData)); //ZeroMemory将指定的内存块清零
	...
	if (m_nMaskingNo == -1)
	{
		...
		SetMaskingPattern(m_nMaskingNo); // 初始化掩膜图形 0
		SetFormatInfoPattern(m_nMaskingNo); //  初始化格式信息

		int nMinPenalty = CountPenalty();  //  初始化掩模结果评价 0

		for (i = 1; i <= 7; ++i)
		{
			SetMaskingPattern(i); // 依次生成另外7种掩膜图形
			SetFormatInfoPattern(i); // 依次设定7种格式信息

			int nPenalty = CountPenalty();    // 依次对这7种掩模结果进行评价

			if (nPenalty < nMinPenalty)     // 选取得分最低的掩模
			{
	...
	SetMaskingPattern(m_nMaskingNo); // 确定最合适的掩模方案
	SetFormatInfoPattern(m_nMaskingNo); // 设置该方案下的格式信息
...
void CQR_Encode::SetFunctionModule()
{
	...
	{
		m_byModuleData[i][6] = (i % 2) == 0 ? '\x30' : '\x20';    //  00011110  00010100
		m_byModuleData[6][i] = (i % 2) == 0 ? '\x30' : '\x20';    //  00011110  00010100
	}
}
...
// 设置版本信息位流，自第7版本开始使用版本信息
// 由6个数据位和12个BCH纠错位组成的18位的序列  P140
...
void CQR_Encode::SetCodeWordPattern()
{
	...
	for (i = 0; i < m_ncAllCodeWord; ++i)
	{
		for (j = 0; j < 8; ++j)
		{
			...
			while (m_byModuleData[x][y] & 0x20); // 与00000010进行与运算
...
// 格式信息为15位，其中有5个数据位，10个是用BCH（15，5）编码计算得到的纠错位  附录P11
//
...
void CQR_Encode::SetFormatInfoPattern(int nPatternNo)
{
	...
	// 例如 M101：00101 bbbbbbbbbb
	// 以数据位串为系数的多项式被生成多项式G（x）=X10+X8+X5+X4+X2+X+1除
	// 所得剩余多项式的系数串应追加到数据位串上形成（15,5）BCH码字符串
	for (i = 0; i < 5; ++i)
	{
		if (nFormatData & (1 << (14 - i)))
		{
			nFormatData ^= (0x0537 << (4 - i)); // 101 0011 0111b
		}
	}

	nFormatData += nFormatInfo << 10;

	// 通过用101010000010010对位串进行异或（XOR）运算进行掩模，
	// 来保证掩模图形和纠错等级的任意组合的格式信息位图不全为0，并使模式2符号与模式1符号自动区别开来。
	nFormatData ^= 0x5412; // 101 0100 0001 0010b
...
// 对掩模结果的评价，分数越高，其结果越不可用
// N1到N4为对不好的特征所罚分数的权重（N1=3，N2=3，N3=40，N4=10）
// i为紧邻的颜色相同模块数大于5的次数，k为符号深色模块所占比率离50%的差距，步长为5%。
// 标准P49
...
int CQR_Encode::CountPenalty()
{
	...
	// 特征1：行/列中相临的模块的颜色相同	条件：模块数 = (5 + i)	分数：N1 + i
	for (i = 0; i < m_nSymbleSize; ++i)
	{
	...
	// 的模块组成的块	 条件：块尺寸 = m×n   分数：N2× (m - 1)×(n - 1)
	// 遍历整个区域，判定2x2区域内的模块颜色情况 即 3×1×1
	...
	// 特征3：在行列中出现1:1:3:1:1（深浅深浅深）图形 	分数：N3

	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize - 6; ++j)
		{
			if (((j == 0) ||				 (! (m_byModuleData[i][j - 1] & 0x11))) && // 检测探测图形1:1:3:1:1
											 (   m_byModuleData[i][j]     & 0x11)   && // 黑 - 1
											 (! (m_byModuleData[i][j + 1] & 0x11))  && // 白 - 1
											 (   m_byModuleData[i][j + 2] & 0x11)   && // 黑
											 (   m_byModuleData[i][j + 3] & 0x11)   && // 黑   3 
											 (   m_byModuleData[i][j + 4] & 0x11)   && // 黑
											 (! (m_byModuleData[i][j + 5] & 0x11))  && // 白 - 1
											 (   m_byModuleData[i][j + 6] & 0x11)   && // 黑 - 1
				((j == m_nSymbleSize - 7) || (! (m_byModuleData[i][j + 7] & 0x11))))   // 
			{
				// 在行列中出现1:1:3:1:1（深浅深浅深）图形
	...
	// 特征3：在纵列中出现1:1:3:1:1（深浅深浅深）图形 	分数：N3
	for (i = 0; i < m_nSymbleSize; ++i)
	{
		for (j = 0; j < m_nSymbleSize - 6; ++j)
		{
			if (((j == 0) ||				 (! (m_byModuleData[j - 1][i] & 0x11))) && // 检测探测图形1:1:3:1:1
											 (   m_byModuleData[j]    [i] & 0x11)   && // 黑 - 1
											 (! (m_byModuleData[j + 1][i] & 0x11))  && // 白 - 1
											 (   m_byModuleData[j + 2][i] & 0x11)   && // 黑
											 (   m_byModuleData[j + 3][i] & 0x11)   && // 黑   3
											 (   m_byModuleData[j + 4][i] & 0x11)   && // 黑
											 (! (m_byModuleData[j + 5][i] & 0x11))  && // 白 - 1
											 (   m_byModuleData[j + 6][i] & 0x11)   && // 黑 - 1
				((j == m_nSymbleSize - 7) || (! (m_byModuleData[j + 7][i] & 0x11))))   // 
			{
				// 在纵列中出现1:1:3:1:1（深浅深浅深）图形
	...
	// 特征4：整个符号中深色模块的比率	条件：50±(5×k)% ~ 50±[5×(k + 1)]%	分数：N4×k
	int nCount = 0;
